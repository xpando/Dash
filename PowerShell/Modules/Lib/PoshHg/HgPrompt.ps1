...
function Write-HgStatus($status = (get-hgStatus)) {
    if ($status) {
        ...
        if($s.ShowTags -and $status.Tags.Length) {
          Write-Host $s.BeforeTagText -NoNewLine -ForegroundColor White
          ...
          $status.Tags | % {
            ...
            if($tagCounter -lt ($status.Tags.Length -1)) {
              Write-Host $s.TagSeparator -NoNewLine -ForegroundColor $s.TagSeparatorColor -BackgroundColor $s.TagBackgroundColor
            }
            
            $tagCounter++;
          }
          
          Write-Host $s.AfterTagText -NoNewLine -ForegroundColor White
        }

        Write-Host $s.BeforeStatusText -NoNewline -ForegroundColor $s.BeforeStatusTextColor
        ...
        Write-Host $s.AfterStatusText -NoNewline -ForegroundColor $s.AfterStatusTextColor
